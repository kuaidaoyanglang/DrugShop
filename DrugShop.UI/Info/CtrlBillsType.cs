...
{
    public partial class CtrlBillsType : UserControl
    {
        public CtrlBillsType()
        {
            InitializeComponent();
        ...
        {
            FrmBillsType frm = new FrmBillsType();
            frm.ShowDialog(this);
        ...
        private void RefreshListView()
        {
            ...
            List<BillsType> modelList = modelService.GetAllBillsTypes();
        ...
        private void tsbDelete_Click(object sender, EventArgs e)
        {
            try
            {
                List<BillsType> selectedModelList = this.GetSelectModelList();
                
                ModelService modelService = new ModelService();
                foreach (BillsType model in selectedModelList)
                {
                    modelService.DeleteBillsType(model, PermissionService.GetCurrentUser().Name);
                }
                this.RefreshListView();
            }
            catch (Exception ex)
        ...
        private void tsbModify_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ModelService modelService = new ModelService();
                FrmBillsType frm = new FrmBillsType(selectedModelList[0]);
                frm.ShowDialog(this);
                if (frm.DialogResult == DialogResult.OK)
                {
                    this.RefreshListView();
        ...
        private void SearchModelList(string searchCond)
        {
            ...
            List<BillsType> modelList = modelServices.SearchBillsType(searchCond);
            this.FillDataGridView(modelList);
        }
        private void FillDataGridView(List<BillsType> modelList)
        {
            this.dataGridView1.Rows.Clear();
            foreach (BillsType model in modelList)
            {
                ...
                row.Cells.Add(cell);
                cell = new DataGridViewTextBoxCell();
                cell.Value = model.IsOut == true ? "出库" : "入库";
                row.Cells.Add(cell);
                cell = new DataGridViewTextBoxCell();
